...
if [ ${IS_FD} == "1" ]; then
  ...
  if [ ${IS_FHC} == "1" ]; then
    ...
    echo "process nr ${proc} head  -${headvar} | tail -${tailvar}"
    (cat ${CAFANA}/InputCAFs.${SAMPLE_NAME}.list | head -${headvar} | tail -${tailvar}) &> FileList_$PROCESS 
    INPFILE=" -N-nu FileList_$PROCESS"
  else
    ...
    (cat ${CAFANA}/InputCAFs.${SAMPLE_NAME}.list | head -${headvar} | tail -${tailvar}) &> FileListRHC_$PROCESS
    INPFILE=" -N-nub FileListRHC_$PROCESS"
  fi
fi
...
if [ ${IS_FD} == "1" ]; then
  ...
  OUTFILENAME=${SAMPLE_NAME}.TESTNDState.${CLUSTER}.${PROCESS}.root
fi
