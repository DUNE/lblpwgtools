...
// Similar to hadd_state but is for summing together state files with 
// the same syst_names hist and adding the total number of events.
// Replaces hadd_cafana, which is terrible for eating too much memory when
// hadding ND state files.
// C. Hasnip 6/5/2021
//
  ...
  while ((key = (TKey*)nextkey())) {
    ...
    if (obj->IsA()->InheritsFrom(TDirectory::Class())) {
      ...
      if (!key2) {
        ...
        target->cd();
        gDirectory->Delete(key->GetName());
        h_new->Write(key->GetName());
      }
      delete h_new;
    }  else {
      target->cd();
      ...
      if (!key3) {
        obj->Write(key->GetName());
      } // else it is already there and don't want to touch it.
      delete obj;      
    }
  }
