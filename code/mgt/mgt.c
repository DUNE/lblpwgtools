...
{ 
  ...
  switch(arguments.chimode){
    ...
    case 17: case 18: case 19:
      ...
      //initialize mult_presmear_effs
      for(int exp=0;exp<glb_num_of_exps;exp++){
        glb_exp e = glb_experiment_list[exp];
        mult_presmear_effs[exp] = (double**) malloc(e->numofchannels*sizeof(double*));
        for(int ch=0;ch<e->numofchannels;ch++){
          mult_presmear_effs[exp][ch]= (double*) malloc(e->simbins*sizeof(double));
        }
      }
      
			for(int exp=0;exp<glb_num_of_exps;exp++){
				int sampbins = glbGetNumberOfSamplingPoints(exp);
        ...
        for(int syst=0; syst<arguments.systs; syst++){
          ...
          printf("rfCols[%d][%d]=%d, penalty[%d][%d]=%d\n",exp,syst,rfCols[exp][syst],exp,syst,penalty[exp][syst]);
		...
		case 1: //sigdcp runtypes (sigdcp.c)
			switch (arguments.runType){
				...
				case 6:
					...
					break;
				case 7:
					ComputeMHSigCurve_v2(osc,0.0);
					break;
				case 8:
					ComputeMHSigCurve_v2(osc,-M_PI/2);
					break;
				case 9:
