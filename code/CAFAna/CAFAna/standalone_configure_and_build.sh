...
cmake ../ -DSRC_ROOT_PARENT=$(readlink -f ../../) -DUSED_UPS=${USE_UPS} -DUSE_GPERFTOOLS=${USE_GPERF} -DCMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE} -DCMAKE_INSTALL_PREFIX=${INSTALL_DIR} -DKNL=${USE_KNL} -DCMAKE_CXX_COMPILER=$(which icpc) -DCMAKE_C_COMPILER=$(which icc) -DCMAKE_CXX_FLAGS="-axMIC-AVX512,AVX"
