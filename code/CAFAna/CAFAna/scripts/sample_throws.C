...
std::vector<std::string> samples = {"FD_nue_FHC", "FD_numu_FHC", "FD_nue_RHC", "FD_numu_RHC", "ND_FHC", "ND_RHC"};

void make_1D_errors(TDirectory* saveDir, std::vector<TH1*> nominal, std::vector<std::vector<TH1*> > thrown_hists){
  ...
  for (uint s=0; s<samples.size(); ++s){
    ...
    TH1D* total_hist = new TH1D((samples[s] + "_rate_ratio").c_str(),
				(samples[s] + "_rate_ratio").c_str(),
				100, 0.5, 1.5);
	...
	TH1D theseThrows((samples[s]+"_"+std::to_string(x)+"_"+std::to_string(y)).c_str(),
                         (samples[s]+"_"+std::to_string(x)+"_"+std::to_string(y)).c_str(),
                         1000, 0, mean*4);
	for (auto v :values) theseThrows.Fill(v);
...
std::vector<TH1*> make_syst_throw(std::string stateFname, osc::IOscCalculatorAdjustable* trueOsc,
                                  SystShifts theseSysts, std::string sampleString){
  ...
  static std::vector<std::unique_ptr<PredictionInterp> > interp_list = GetPredictionInterps(stateFname, GetListOfSysts());
  ...
  std::vector<TH1*> ret;
  ret.push_back(GetMCSystTotal(&predFDNueFHC, trueOsc, theseSysts, (samples[0]+std::to_string(haXX0r)).c_str(), pot_fd_fhc_nue));
  ret.push_back(GetMCSystTotal(&predFDNumuFHC, trueOsc, theseSysts, (samples[1]+std::to_string(haXX0r)).c_str(), pot_fd_fhc_numu));
  ret.push_back(GetMCSystTotal(&predFDNueRHC, trueOsc, theseSysts, (samples[2]+std::to_string(haXX0r)).c_str(), pot_fd_rhc_nue));
  ret.push_back(GetMCSystTotal(&predFDNumuRHC, trueOsc, theseSysts, (samples[3]+std::to_string(haXX0r)).c_str(), pot_fd_rhc_numu));
  ret.push_back(GetMCSystTotal(&predNDNumuFHC, trueOsc, theseSysts, (samples[4]+std::to_string(haXX0r)).c_str(), pot_nd_fhc));
  ret.push_back(GetMCSystTotal(&predNDNumuRHC, trueOsc, theseSysts, (samples[5]+std::to_string(haXX0r)).c_str(), pot_nd_rhc));
  haXX0r += 1;
...
void throw_errors(std::string stateFname, std::string sampleString, TDirectory* saveDir, std::vector<TH1*> nom_vect,
		  std::vector<const ISyst *> systlist, std::vector<std::string> systtokeep, 
		  osc::IOscCalculatorAdjustable* trueOsc, TMatrixDSym* chol=NULL){
  ...
  for (int i=0; i<nSysts; ++i){
    ...
    if (std::find(systtokeep.begin(), systtokeep.end(), systlist[i]->ShortName()) == systtokeep.end()){
      onoffvect[i] = 0;
  ...
  for (int i=0; i<nthrows; ++i){
    ...
    if (chol != NULL){
      for (int i=0; i<nSysts; ++i) gaus_vect[i] = gRandom->Gaus(0.0, 1.0);
      TVector temp_vect = (*chol)*gaus_vect;
      for (int i=0; i<nSysts; ++i) throw_vect[i] = temp_vect[i]*onoffvect[i];
    } else {
      throw_vect = TVector(nSysts);
      for (int i=0; i<nSysts; ++i) throw_vect[i] = GetBoundedGausThrow(systlist[i]->Min(), systlist[i]->Max())*onoffvect[i];
    }
    ...
    for (int i=0; i<nSysts; ++i) {
      ...
      //std::cout << "Setting " << systlist[i]->ShortName() << " to " << throw_vect[i] << std::endl;
    }

    fit_vect.push_back(make_syst_throw(stateFname, trueOsc, throwSyst, sampleString));
  }
...
void sample_throws(std::string stateFname = "common_state_mcc11v3.root",
		   std::string outputFname = "covar_various_asimov.root",
		   std::string systSet = "flux", std::string sampleString = "ndfd",
		   std::string penaltyString = "", std::string asimov_set=""){
  ...
  for (auto s : systlist) allsystnames.push_back(s->ShortName());
  ...
  std::vector<TH1*> nom_vect =  make_syst_throw(stateFname, trueOsc, kNoShift, sampleString);
  ...
  throw_errors(stateFname, sampleString, prefit_dir, nom_vect,
  ...
  throw_errors(stateFname, sampleString, prefit_flux_dir, nom_vect,
               systlist, flux_list, trueOsc);
  
  std::cout << "Prefit fd_det uncertainties..." << std::endl;
  TDirectory* prefit_fd_det_dir = fout->mkdir("prefit_fd_det");
  throw_errors(stateFname, sampleString, prefit_fd_det_dir, nom_vect,
               systlist, fd_det_list, trueOsc);

  std::cout << "Prefit xsec_qe uncertainties..." << std::endl;
  TDirectory* prefit_xsec_qe_dir = fout->mkdir("prefit_xsec_qe");
  throw_errors(stateFname, sampleString, prefit_xsec_qe_dir, nom_vect,
               systlist, xsec_qe_list, trueOsc);
  ...
  throw_errors(stateFname, sampleString, prefit_xsec_dir, nom_vect,
               systlist, all_xsec_list, trueOsc);
  ...
  TMatrixDSym sub_covar = covar->GetSub(oscVars.size(), covar->GetNrows()-1, oscVars.size(), covar->GetNrows()-1);
  ...
  TMatrixDSym* dec_mat = new TMatrixDSym(sub_covar.GetNrows(), LU.GetU().GetMatrixArray(), "");
  TH2D hist_chol = TH2D(*dec_mat);
  hist_chol.SetName("hist_chol");
  hist_chol.Write();

  std::cout << "Postfit all uncertainties..." << std::endl;
  throw_errors(stateFname, sampleString, postfit_dir, nom_vect,
               systlist, allsystnames, trueOsc, dec_mat);
  
  std::cout << "Postfit flux uncertainties..." << std::endl;
  TDirectory* postfit_flux_dir = fout->mkdir("postfit_flux");
  throw_errors(stateFname, sampleString, postfit_flux_dir, nom_vect,
               systlist, flux_list, trueOsc, dec_mat);

  std::cout << "Postfit fd_det uncertainties..." << std::endl;
  TDirectory* postfit_fd_det_dir = fout->mkdir("postfit_fd_det");
  throw_errors(stateFname, sampleString, postfit_fd_det_dir, nom_vect,
	       systlist, fd_det_list, trueOsc, dec_mat);
  ...
  throw_errors(stateFname, sampleString, postfit_xsec_dir, nom_vect,
               systlist, all_xsec_list, trueOsc, dec_mat);
