...
if [ "${LATEST_EXISTS}" == "" ] || [ "${DO_BUILD}" == "1" ]; then
  ...
  echo -e "#!/bin/bash\nyum install -y --setopt=tsflags=nodocs svn which make redhat-lsb-core glibc-devel automake libtool && yum clean all;cd /opt;source /cvmfs/dune.opensciencegrid.org/products/dune/setup_dune.sh;git clone ${REPO_URL} lblpwgtools;cd lblpwgtools;git checkout ${BRANCH_NAME};cd code/CAFAna/CAFAna;./standalone_configure_and_build.sh -j ${NCORES} -u ${ROPT}${GOPT}-I /opt/CAFAna/; cat /build_scripts/CAFEnvWrapper.sh > /opt/CAFAna/CAFEnvWrapper.sh; chmod -R 775 /opt/CAFAna; if [ \"${DO_KNL}\" == \"1\" ]; then ./standalone_configure_and_build.sh -f -j ${NCORES} --knl -u ${ROPT}${GOPT}-I /opt/CAFAna_knl/; cat /build_scripts/CAFEnvWrapper_knl.sh > /opt/CAFAna_knl/CAFEnvWrapper.sh; chmod -R 775 /opt/CAFAna_knl; fi; if [ \"${REMOVE_SOURCE}\" == \"1\" ]; then rm -rf /opt/lblpwgtools; fi; if [ -e /statefiles_in ]; then cp -r /statefiles_in /statefiles; chmod -R 744 /statefiles; fi" > CAFBuildScript.sh
