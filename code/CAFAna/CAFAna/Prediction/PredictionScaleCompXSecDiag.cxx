...
namespace ana
{
  std::vector<std::vector<double>> PredictionScaleCompXSecDiag::fgCoeffs;
  ...
  //----------------------------------------------------------------------
  PredictionScaleCompXSecDiag::
  PredictionScaleCompXSecDiag(SpectrumLoaderBase& loader,
                              const HistAxis&     axis,
                              Cut                 cut,
                              const SystShifts&   shift,
                              const Var&          wei)
    : PredictionScaleComp(loader, axis, cut, GetDUNEXSecSysts(), shift, wei),
  ...
  PredictionScaleCompXSecDiag(DUNERunPOTSpectrumLoader& loaderBeam,
                              ...
                              const SystShifts&   shift,
                              const Var&          wei)
    : PredictionScaleComp(loaderBeam, loaderNue, loaderNuTau, loaderNC,
                          axis, cut, GetDUNEXSecSysts(), shift, wei),
      fDiagSysts(GetDUNEXSecDiagSysts())
  {
    InitCoeffs();
  }
  ...
  //----------------------------------------------------------------------
  PredictionScaleCompXSecDiag::PredictionScaleCompXSecDiag(const PredictionScaleComp* p)
    : PredictionScaleComp(*p),
      fDiagSysts(GetDUNEXSecDiagSysts())
  {
    InitCoeffs();
  }
  ...
  //----------------------------------------------------------------------
  Spectrum PredictionScaleCompXSecDiag::
  PredictSyst(osc::IOscCalculator* calc,
              const SystShifts&    shift) const
  {
    ...
    if(!anyDiagSyst) return PredictionScaleComp::PredictSyst(calc, shift);

    // We'll translate from our diagonalized shifts to the underlying "physics"
    // shifts and then pass along to our parent class.
    return PredictionScaleComp::PredictSyst(calc, Undiagonalize(shift));
  }

  //----------------------------------------------------------------------
  SystShifts PredictionScaleCompXSecDiag::
  Undiagonalize(const SystShifts& s) const
  {
  ...
  //----------------------------------------------------------------------
  std::unique_ptr<PredictionScaleCompXSecDiag> PredictionScaleCompXSecDiag::
  LoadFrom(TDirectory* dir)
  {
    return std::unique_ptr<PredictionScaleCompXSecDiag>(new PredictionScaleCompXSecDiag(PredictionScaleComp::LoadFrom(dir).release()));
  }

  //----------------------------------------------------------------------
  void PredictionScaleCompXSecDiag::InitCoeffs()
  {
