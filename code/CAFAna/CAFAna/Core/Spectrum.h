...
public:
  ...
  Spectrum(const std::string &label, const Binning &bins,
           ...
           const SystShifts &shift = kNoShift, const Var &wei = kUnweighted);

  /// The only \ref MultiVar variant available
  Spectrum(const std::string &label, const Binning &bins,
           SpectrumLoaderBase &loader, const MultiVar &var, const Cut &cut,
           const SystShifts &shift = kNoShift, const Var &wei = kUnweighted);

  Spectrum(SpectrumLoaderBase &loader, const HistAxis &axis, const Cut &cut,
           const SystShifts &shift = kNoShift, const Var &wei = kUnweighted);

  Spectrum(const std::string &label, const Binning &bins,
           ESparse sparse = kDense);
  Spectrum(const std::string &label, double pot, double livetime,
           const Binning &bins);

  /// Copies \a h
  Spectrum(TH1 const *h, const std::vector<std::string> &labels,
           const std::vector<Binning> &bins, double pot, double livetime);

  /// Takes possession of \a h
  Spectrum(std::unique_ptr<TH1D> h, const std::vector<std::string> &labels,
           const std::vector<Binning> &bins, double pot, double livetime);
  ...
  Spectrum(const std::string &label, SpectrumLoaderBase &loader,
           ...
           const Var &vary, const Cut &cut, const SystShifts &shift = kNoShift,
           const Var &wei = kUnweighted);

  /// 2D Spectrum taking 2 HistAxis
  Spectrum(SpectrumLoaderBase &loader, const HistAxis &xAxis,
           const HistAxis &yAxis, const Cut &cut,
           const SystShifts &shift = kNoShift, const Var &wei = kUnweighted);

  Spectrum(const std::string &xLabel, const std::string &yLabel,
           SpectrumLoaderBase &loader, const Binning &binsx, const Var &varx,
           const Binning &binsy, const Var &vary, const Cut &cut,
           const SystShifts &shift = kNoShift, const Var &wei = kUnweighted);
  ...
  Spectrum(const std::string &label, SpectrumLoaderBase &loader,
           ...
           const Var &wei = kUnweighted, ESparse sparse = kDense);
  ...
  Spectrum(const std::vector<std::string> &labels,
           const std::vector<Binning> &bins, ESparse sparse = kDense);
  ...
  void FillFromHistogram(TH1 const *h);
  ...
  TH1D *ToTH1(double exposure, Color_t col = kBlack, Style_t style = kSolid,
  ...
  /// \param exposure POT or livetime (seconds)
  /// \param expotype How to interpret exposure (kPOT (default) or kLivetime)
  TH1D *ToTH1(double exposure, EExposureType expotype,
              EBinType bintype = kBinContent) const;
  ...
  /// Function decides what is the appropriate projection based on fBins, and
  /// does that
  TH1 *ToTHX(double exposure, bool force1D = false,
  ...
  /// Change the internal POT while maintaining the Norm/POT ratio, any
  /// predictions should be unaffected, this is probably uncessesary as
  /// PlusEqualsHelper scales by POT.
...
// Commutative
inline Spectrum operator*(const Ratio &lhs, const Spectrum &rhs) {
  return rhs * lhs;
}
inline Spectrum operator/(const Ratio &lhs, const Spectrum &rhs) {
  return rhs / lhs;
