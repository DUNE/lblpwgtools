...
std::unordered_multimap<std::pair<int, int>, std::unique_ptr<TH2D>>
    HistCache::fgMap2D;

#ifdef HISTCACHE_EXTRA_CARE
std::set<TH1D *> HistCache::fgSet;
std::set<TH2D *> HistCache::fgSet2D;
#endif
...
TH1D *HistCache::New(const std::string &title, const Binning &bins) {
  ...
  TH1D *hret = MakeTH1D(UniqueName().c_str(), title.c_str(), bins);
#ifdef HISTCACHE_EXTRA_CARE
  fgSet.insert(hret);
#endif
  return hret;
...
//---------------------------------------------------------------------
TH2D *HistCache::NewTH2D(const std::string &title, const Binning &xbins,
                         const Binning &ybins) {
  ++fgOut;
  ...
  TH2D *hret = MakeTH2D(UniqueName().c_str(), title.c_str(), xbins, ybins);
#ifdef HISTCACHE_EXTRA_CARE
  fgSet2D.insert(hret);
#endif
  return hret;
...
//---------------------------------------------------------------------
TH2D *HistCache::NewTH2D(const std::string &title, const TAxis *xbins,
                         const TAxis *ybins) {
...
//---------------------------------------------------------------------
TH2D *HistCache::Copy(const TH2D *h, const Binning &xbins,
                      const Binning &ybins) {
...
void HistCache::Delete(TH2D *&h, int binidx, int binidy) {
  ...
  if (binidx < 0)
    binidx = Binning::FromTAxis(h->GetXaxis()).ID();
  if (binidy < 0)
    binidy = Binning::FromTAxis(h->GetYaxis()).ID();

  fgMap2D.emplace(std::pair<int, int>(binidx, binidy),
...
void HistCache::PrintStats() {
  ...
  std::cout << "Gave out " << fgOut << " histograms, got back " << fgIn
            << " of them (" << fgOut - fgIn << " still out, totalling "
            << fgMemHandedOut << " bytes), in " << keys.size()
            << " different shapes." << std::endl
            << "Holding " << fgMap.size() + fgMap2D.size()
            << " histograms for an estimated memory usage of " << fgEstMemUsage
            << " bytes." << std::endl;
}

#ifdef HISTCACHE_EXTRA_CARE
bool HistCache::IsFromCache(TH1D *h) { return fgSet.count(h); }
bool HistCache::IsFromCache(TH2D *h) { return fgSet2D.count(h); }
#endif

void HistCache::Adopt(TH1D *h) {
  ++fgOut;
  fgMemHandedOut += 16 * h->GetNbinsX();
  fgEstMemUsage -= 16 * h->GetNbinsX();
#ifdef HISTCACHE_EXTRA_CARE
