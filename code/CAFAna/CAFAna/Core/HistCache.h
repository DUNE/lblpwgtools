...
// Enable if you want much more stringent checking of histograms
// handed out and received back.
// #define HISTCACHE_EXTRA_CARE
...
public:
  ...
  /// WARNING - the first two TH2D functions are kept for backward
  /// compatibility, they take kTrueEnergyBins on the y-axis.
  ...
  static TH2D *NewTH2D(const std::string &title, const Binning &xbins,
                       const Binning &ybins);
  static TH2D *NewTH2D(const std::string &title, const TAxis *xbins,
                       const TAxis *ybins);
  ...
  static bool IsFromCache(TH1D *);
  static bool IsFromCache(TH2D *);
#endif
  static void Adopt(TH1D *);
  static void Adopt(TH2D *);
...
protected:
  ...
  static std::unordered_multimap<std::pair<int, int>, std::unique_ptr<TH2D>>
      fgMap2D;

#ifdef HISTCACHE_EXTRA_CARE
  static std::set<TH1D *> fgSet;
  static std::set<TH2D *> fgSet2D;
#endif
