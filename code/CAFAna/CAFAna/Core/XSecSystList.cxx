...
static std::vector<XSecDialDescriptor> const XSecSystDials = {
    ...
    {"Mnv2p2hGaussEnhancement", kFitSyst, kContinuous, kStandardRange},
...
int GetXSecSystIndex(std::string const &name) {
  ...
  if (!cache.count(name)) {
    int found = -1;
    for (int it = 0; it < XSecSystDials.size(); ++it) {
...
std::pair<double, double> GetXSecSystLimits(int index) {
  assert((index > 0) && (index < XSecSystDials.size()));
