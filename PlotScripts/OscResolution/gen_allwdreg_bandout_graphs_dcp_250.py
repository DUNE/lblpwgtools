...
if __name__=="__main__":
	...
	for n in range(len(targ_dchisqs)):
		for i in range(len(ssth23_vals)):
			...
			tf = ROOT.TFile("1dscan_graphs/dcp_250/1dscan_graphs_ssth23=%lf.root" % ssth23_vals[i])
		...
		#		graphs[j].SetTitle(";True #delta_{cp};Allowed #delta_{cp}")	
			...
			for k in range(len(dcp_names)):
				...
				if len(inters) == 2:
					...
					if tr_dcp_placement == 1:
						...
						cont_upper_band = True
						if not lower_top_graphs.GetListOfGraphs() != ROOT.nullptr: cont_lower_band = False
						...
						cont_lower_band = True
						if not upper_top_graphs.GetListOfGraphs() != ROOT.nullptr: cont_upper_band = False
			...
			outf = ROOT.TFile("1dscan_graphs/dcp_250/allwdreg_bandout_graphs_ssth23=%lf_%s.root" % (ssth23_vals[i],level_names[n]), "RECREATE")
