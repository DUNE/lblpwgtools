...
public:
  // Not having default constructors is just a pain, and especially unneccesary
  // when all of the data members are copyable... (evidenced by the other
  // constructors taking const references to everything)
  GenericHistAxis() {}

  GenericHistAxis(const std::string &label, const Binning &bins, const T &var);

  GenericHistAxis(const std::vector<std::string> &labels,
                  const std::vector<Binning> &bins, const std::vector<T> &vars);

  GenericHistAxis(const std::string &labelX, const Binning &binsX,
                  const T &varX, const std::string &labelY,
                  const Binning &binsY, const T &varY);

  /// Shortcut for simple binnings
  GenericHistAxis(const std::string &label, int nx, double x0, double x1,
                  const T &var);

  /// Shortcut for simple binnings
  GenericHistAxis(const std::string &labelX, int nx, double x0, double x1,
                  const T &varX, const std::string &labelY, int ny, double y0,
                  double y1, const T &varY);
