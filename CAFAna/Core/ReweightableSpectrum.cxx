...
ReweightableSpectrum::ReweightableSpectrum(SpectrumLoaderBase &loader,
  ...
  if (xbins.IsSimple() && ybins.IsSimple())
    fHist = new TH2D(name.c_str(), "", xbins.NBins(), xbins.Min(), xbins.Max(),
  ...
  if (!xbins.IsSimple() && ybins.IsSimple())
    fHist = new TH2D(name.c_str(), "", xbins.NBins(), &xbins.Edges()[0],
                     ybins.NBins(), ybins.Min(), ybins.Max());
  ...
  loader.AddReweightableSpectrum(*this, recoAxis.GetMultiDVar(), cut, shift,
                                 wei);
}
...
ReweightableSpectrum::ReweightableSpectrum(const Var &rwVar,
                                           ...
                                           double pot, int nbinsx, double xmin,
                                           double xmax, int nbinsy, double ymin,
                                           double ymax)
    : ReweightableSpectrum(xlabel, Binning::Simple(nbinsx, xmin, xmax), rwVar) {
  DontAddDirectory guard;

  fHist = new TH2D(UniqueName().c_str(), "", nbinsx, xmin, xmax, nbinsy, ymin,
                   ymax);
...
ReweightableSpectrum::ReweightableSpectrum(
    const Var &rwVar, TH2 *h, const std::vector<std::string> &labels,
  ...
  if (className == "TH2D") {
    ...
    assert(bool(ax->GetXbins()->GetArray()) ==
           bool(ay->GetXbins()->GetArray()));

    if (ax->GetXbins()->GetArray()) {
      fHist = new TH2D(UniqueName().c_str(), "", ax->GetNbins(),
                       ax->GetXbins()->GetArray(), ay->GetNbins(),
                       ay->GetXbins()->GetArray());
    } else {
      fHist =
          new TH2D(UniqueName().c_str(), "", ax->GetNbins(), ax->GetXmin(),
                   ax->GetXmax(), ay->GetNbins(), ay->GetXmin(), ay->GetXmax());
    }
...
//----------------------------------------------------------------------
ReweightableSpectrum::ReweightableSpectrum(
    const Var &rwVar, std::unique_ptr<TH2D> h,
    const std::vector<std::string> &labels, const std::vector<Binning> &bins,
...
ReweightableSpectrum::~ReweightableSpectrum() {
  if (fHist && fHist->GetDirectory()) {
    ...
    if (once) {
      once = false;
      std::cerr << "ReweightableSpectrum's fHist is associated with a "
                   "directory. How did that happen?"
                << std::endl;
    }
  }
...
//----------------------------------------------------------------------
ReweightableSpectrum &ReweightableSpectrum::
operator=(const ReweightableSpectrum &rhs) {
  if (this == &rhs)
    return *this;
...
Spectrum ReweightableSpectrum::WeightedByErrors(const TH1 *ws) const {
  // This function is similar to WeightedBy but lets TH1 keep track of the
...
ReweightableSpectrum::PlusEqualsHelper(const ReweightableSpectrum &rhs,
  ...
  if ((!fPOT && !fLivetime) || (!rhs.fPOT && !rhs.fLivetime)) {
    std::cout
        << "Error: can't sum ReweightableSpectrum with no POT or livetime."
        << fPOT << " " << rhs.fPOT << std::endl;
    //      abort();
    return *this;
  }

  if (!fLivetime && !rhs.fPOT) {
    std::cout << "Error: can't sum ReweightableSpectrum with POT (" << fPOT
              << ") but no livetime and ReweightableSpectrum with livetime ("
  ...
  if (!fPOT && !rhs.fLivetime) {
    std::cout << "Error: can't sum ReweightableSpectrum with livetime ("
              << fLivetime
              << " sec) but no POT and ReweightableSpectrum with POT ("
...
//----------------------------------------------------------------------
ReweightableSpectrum ReweightableSpectrum::
operator+(const ReweightableSpectrum &rhs) const {
...
//----------------------------------------------------------------------
void ReweightableSpectrum::Clear() { fHist->Reset(); }
...
ReweightableSpectrum::LoadFrom(TDirectory *dir) {
  ...
  return std::make_unique<ReweightableSpectrum>(kUnweighted, spect, labels,
                                                bins, hPot->GetBinContent(1),
