...
MCEffCorrection::~MCEffCorrection() {
  for (auto &pred : NDUnselPredInterps)
    pred = nullptr;
  for (auto &pred : NDSelPredInterps)
    pred = nullptr;
  for (auto &pred : FDUnselPredInterps)
    pred = nullptr;
  for (auto &pred : FDSelPredInterps)
    pred = nullptr;
}

//----------------------------------------------------

void MCEffCorrection::Initialize(
    std::pair<PredictionInterp const *, size_t> NDunsel_293kA,
...
void MCEffCorrection::CalcEfficiency(
    osc::IOscCalc *calc, HistAxis const &axis, ana::SystShifts shift,
    Flavors::Flavors_t NDflav, Flavors::Flavors_t FDflav,
    Current::Current_t curr, Sign::Sign_t NDsign, Sign::Sign_t FDsign) const {
  ...
  // hNDUnselected_293kA =
  // std::unique_ptr<TH2D>(dynamic_cast<TH2D*>(sNDunselected_293kA.ToTH2(1)));

  // Analysis axis could be 2D, so put into RWSpec so we can have it projected
  // into 1D trueaxis
  Eigen::ArrayXXd NDunsel_293kA =
      ConvertArrayToMatrix(sNDunselected_293kA.GetEigen(1),
                           sNDunselected_293kA.GetBinnings())
          .array()
          .block(1, 1, NDefficiency_293kA.rows(), NDefficiency_293kA.cols());
  Eigen::ArrayXXd NDunsel_280kA =
      ConvertArrayToMatrix(sNDunselected_280kA.GetEigen(1),
                           sNDunselected_280kA.GetBinnings())
          .array()
          .block(1, 1, NDefficiency_280kA.rows(), NDefficiency_280kA.cols());
  ...
  // Maybe you want this for debugging
  // hNDSelected_293kA =
  // std::unique_ptr<TH2D>(dynamic_cast<TH2D*>(sNDselected_293kA.ToTH2(1)));

  Eigen::ArrayXXd NDsel_293kA =
      ConvertArrayToMatrix(sNDselected_293kA.GetEigen(1),
                           sNDselected_293kA.GetBinnings())
          .array()
          .block(1, 1, NDefficiency_293kA.rows(), NDefficiency_293kA.cols());
  Eigen::ArrayXXd NDsel_280kA =
      ConvertArrayToMatrix(sNDselected_280kA.GetEigen(1),
  ...
  Eigen::ArrayXd vFDunselected =
      sFDunselected.GetEigen(1).segment(1, FDefficiency.size());
  Eigen::ArrayXd vFDselected =
      sFDselected.GetEigen(1).segment(1, FDefficiency.size());
...
//----------------------------------------------------------
size_t MCEffCorrection::GetNDConfigFromPred(Flavors::Flavors_t NDflav,
                                            Sign::Sign_t NDsign,
                                            bool is280kA) const {
  size_t conf;
...
size_t MCEffCorrection::GetFDConfigFromPred(Flavors::Flavors_t FDflav,
...
void MCEffCorrection::Write(TDirectory *dir) const {
  ...
  osc::NoOscillations no;
  size_t conf = kND_293kA_nu_numu;
  if (!NDUnselPredInterps.at(conf))
    conf = kND_293kA_nub_numub;
  if (!NDUnselPredInterps.at(conf)) {
  ...
  size_t conf_280kA = kND_280kA_nu_numu;
  if (!NDUnselPredInterps.at(conf_280kA))
    conf_280kA = kND_280kA_nub_numub;
  if (!NDUnselPredInterps.at(conf_280kA)) {
    std::cout << "[ERROR] Cannot write efficiency." << std::endl;
    abort();
  }
  ...
  LabelsAndBins oa_axis =
      (NDPred.GetBinnings().size() == 2)
          ? LabelsAndBins(NDPred.GetLabels().at(1), NDPred.GetBinnings().at(1))
          : LabelsAndBins(NDPred.GetLabels().at(2), NDPred.GetBinnings().at(2));
  LabelsAndBins oa280kA_axis =
      (NDPred_280kA.GetBinnings().size() == 2)
          ? LabelsAndBins(NDPred_280kA.GetLabels().at(1),
                          NDPred_280kA.GetBinnings().at(1))
          : LabelsAndBins(NDPred_280kA.GetLabels().at(2),
                          NDPred_280kA.GetBinnings().at(2));

  Eigen::MatrixXd NDEff293kA_mat = Eigen::MatrixXd::Zero(
      NDefficiency_293kA.rows() + 2, NDefficiency_293kA.cols() + 2);
  NDEff293kA_mat.block(1, 1, NDefficiency_293kA.rows(),
                       NDefficiency_293kA.cols()) = NDefficiency_293kA.matrix();

  ReweightableSpectrum NDEff293kA_rws(std::move(NDEff293kA_mat), ana_axis,
                                      oa_axis, 1, 0);

  Eigen::MatrixXd NDEff280kA_mat = Eigen::MatrixXd::Zero(
      NDefficiency_280kA.rows() + 2, NDefficiency_280kA.cols() + 2);
  NDEff280kA_mat.block(1, 1, NDefficiency_280kA.rows(),
                       NDefficiency_280kA.cols()) = NDefficiency_280kA.matrix();

  ReweightableSpectrum NDEff280kA_rws(std::move(NDEff280kA_mat), ana_axis,
                                      oa280kA_axis, 1, 0);
