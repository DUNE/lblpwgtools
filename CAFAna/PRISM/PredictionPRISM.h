...
public:
  ...
  PredictionPRISM(const HistAxis &AnalysisAxisND,
                  const HistAxis &AnalysisAxisFD, const HistAxis &NDOffAxis,
                  const HistAxis &ND280kAAxis, const HistAxis &TrueAnalysisAxis,
  ...
  std::map<PRISMComponent, Spectrum> PredictGaussianFlux_forNDtarget(
      double mean, double width, ana::SystShifts shift = kNoShift,
  ...
  NDFD_Matrix const *Get_NDFD_Matrix() const { return fNDFD_Matrix; }
...
protected:
  ...
  struct _Predictions {
    ...
    struct _FD {
      ...
      std::unique_ptr<PredictionInterp>
          numu_ccinc_sel_sig_apposc_numode_true_e_nu;
      std::unique_ptr<PredictionInterp>
          numubar_ccinc_sel_sig_apposc_nubmode_true_e_nu;

      std::unique_ptr<PredictionInterp>
          nue_ccinc_sel_sig_apposc_numode_true_e_nu;
      std::unique_ptr<PredictionInterp>
  ...
  GetNDData_right_sign_numu(PRISM::BeamMode NDBM, int kA = 293,
                            bool shift = false) const;
  std::unique_ptr<PRISMReweightableSpectrum> &
  GetNDData_right_sign_nue(PRISM::BeamMode NDBM, int kA = 293,
                           bool shift = false) const;
  std::unique_ptr<PRISMReweightableSpectrum> &
  GetNDData_wrong_sign_numu(PRISM::BeamMode NDBM, int kA = 293,
                            bool shift = false) const;
  std::unique_ptr<PRISMReweightableSpectrum> &
  GetNDData(PRISM::BeamChan NDChannel = PRISM::kNumu_Numode, int kA = 293,
            bool shift = false) const;
  bool HaveNDData(PRISM::BeamChan NDChannel = PRISM::kNumu_Numode, int kA = 293,
                  bool shift = false) const;
  ...
  // std::unique_ptr<PredictionInterp> &
  // GetNDTargetPrediction(PRISM:: BeamChan NDChannel = PRISM::kNue_I_Numode)
  // const; //eran adding

  // bool HaveNDTargetPrediction(PRISM::BeamChan NDChannel =
  // PRISM::kNue_I_Numode) const; //eran
  ...
  // Prediction for unselected FD numus to be appearance oscillated in true nu
  // energy
  std::unique_ptr<PredictionInterp> &
  GetFDNonSwapAppOscPredictionTrueEnu(PRISM::BeamMode FDBM) const;
  // Prediction for unselected FD nues to be appearance oscillated in true nu
  // energy
  std::unique_ptr<PredictionInterp> &
  ...
  // so we only want to add MC corrections to PRISMPred (which has
  // fAnalysisAxisND) if the axes are the same.
  bool fAxisAgreement;
