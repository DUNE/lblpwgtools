...
void PRISMPrediction(fhicl::ParameterSet const &pred) {
  ...
  std::pair<double, double> gauss_flux =
      pred.get<std::pair<double, double>>("gauss_flux", {1.5, 0.15});
  ...
  for (auto const &ch : Channels) {
    ...
    std::cout<<" **** osc pars: "<<" th 23: "<<calc->GetTh23()<<" th 12: "<<calc->GetTh12()<<" th 13: "<<calc->GetTh13()<<" dmsq32: "<<calc->GetDmsq32()<<" dmsq 21: "<<calc->GetDmsq21()<<" dCP = "<<calc->GetdCP()<<" L = "<< calc->GetL()<<" rho: "<<calc->GetRho()<<std::endl;
