...
void MergeFits(TDirectory *target, TFile *source, std::string param, int Nbins) {
  ...
  while ((key = (TKey*)nextkey())) {
	...
	gDirectory->Delete("h_dChi2_Total;*");
      ...
      delete h_fit; 
    } else if(keyname.find("flux_Sept21") != std::string::npos && obj->IsA() == TH1D::Class()){
       std::cout<<" Found flux Sep21 syst shift histogram " << std::endl;  

       TH1 *h_fit2 = (TH1*)obj;
       h_fit2->SetDirectory(nullptr);
       target->cd();
       TKey *key_g = (TKey*)gDirectory->GetListOfKeys()->FindObject(keyname.c_str());
       if (!key_g) {
         std::unique_ptr<TH1D> h_totalfitSyst = std::make_unique<TH1D>(
                                              ("Total_" + keyname).c_str(), ("Total_" + keyname).c_str(),
                                              Nbins,
                                              *lowlim, *highlim);

         for (int bin = 1; bin <= h_fit2->GetNbinsX(); bin++) {
           double bincentre_x = h_fit2->GetXaxis()->GetBinCenter(bin);
           double bincontent = h_fit2->GetBinContent(bin);
           std::cout << "flux shift = " << bincontent << std::endl;
           int binx_tot = h_totalfitSyst->GetXaxis()->FindBin(bincentre_x);
           if (std::isnormal(bincontent)) {
             h_totalfitSyst->SetBinContent(binx_tot, bincontent); 
           } else {
             h_totalfitSyst->SetBinContent(binx_tot, 0);
           }
         }
         h_totalfitSyst.release()->Write(keyname.c_str());      
      } else {  

         std::cout << "Updating previous 1D flux systematics shift." << std::endl;
         TH1D *h_updateSyst = (TH1D*)target->Get(keyname.c_str());
         
         for (int bin = 1; bin <= h_fit2->GetNbinsX(); bin++) {
           double bincentre_x = h_fit2->GetXaxis()->GetBinCenter(bin);
           double bincontent = h_fit2->GetBinContent(bin);
           std::cout << "flux shift = " << bincontent << std::endl;
           int binx_tot = h_updateSyst->GetXaxis()->FindBin(bincentre_x);
           if (std::isnormal(bincontent)) {
             h_updateSyst->SetBinContent(binx_tot, bincontent);
           } else {
             h_updateSyst->SetBinContent(binx_tot, 0);
           }
         }
         
         target->cd();
         gDirectory->Delete((keyname+";*").c_str());
	 h_updateSyst->Write(keyname.c_str());
         delete h_updateSyst;
      }
      delete h_fit2; 
...
void usage() {
  std::cout << "Usage: hadd_fitsWithFluxSystShift scan_param output.root input1.root input2.root ..." << std::endl
