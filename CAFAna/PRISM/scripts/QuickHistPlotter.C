...
void QuickHistPlotter()
{
  ...
  vector<TString> histeffunsel;
  vector<TString> histeffsel;
  histeffunsel.clear();
  histeffsel.clear();
  histeffunsel.push_back("MCEfficiency/NDUnselected_293kA");
  histeffunsel.push_back("MCEfficiency/NDUnselected_280kA");
  histeffunsel.push_back("MCEfficiency/FDUnselected");
  histeffsel.push_back("MCEfficiency/NDSelected_293kA"); // 2d eff = sel/unsel
  histeffsel.push_back("MCEfficiency/NDSelected_280kA");
  histeffsel.push_back("MCEfficiency/FDSelected");
  ...
  if ( histeffunsel.size() == histeffsel.size() ) {

    for ( size_t i = 0; i < histeffunsel.size(); i++ ) {

      if ( debug ) std::cout << "looking at histogram: " << histeffunsel[i] << " and " << histeffsel[i] << std::endl;

      //
      // Clone histogram
      //

      TH2F *nomunsel = (TH2F*)file0->Get( TString::Format("numu_EvMatch_nom/%s/%s",   channame.Data(), histeffunsel[i].Data()) )->Clone("nomunsel");
      TH2F *n1unsel  = (TH2F*)file0->Get( TString::Format("numu_EvMatch_%s_-1/%s/%s", systname.Data(), channame.Data(), histeffunsel[i].Data()) )->Clone("n1unsel");
      TH2F *p1unsel  = (TH2F*)file0->Get( TString::Format("numu_EvMatch_%s_1/%s/%s",  systname.Data(), channame.Data(), histeffunsel[i].Data()) )->Clone("p1unsel");
