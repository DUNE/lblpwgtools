...
NDFD_Matrix::NDFD_Matrix() {
  for (size_t conf = 0; conf < kNPRISMConfigs; conf++)
    NDPredInterps.push_back(nullptr);
  for (size_t conf = 0; conf < kNPRISMFDConfigs; conf++)
    FDPredInterps.push_back(nullptr);
...
NDFD_Matrix::~NDFD_Matrix() {
  for (auto &pred : NDPredInterps)
    pred = nullptr;
  for (auto &pred : FDPredInterps)
    pred = nullptr;
}
...
Eigen::MatrixXd NDFD_Matrix::GetNDMatrix() const { return hMatrixND; }

//-----------------------------------------------------

Eigen::MatrixXd NDFD_Matrix::GetFDMatrix() const { return hMatrixFD; }
...
Eigen::MatrixXd NDFD_Matrix::GetCovMat_293kA() const { return hCovMat_293kA; }
Eigen::MatrixXd NDFD_Matrix::GetCovMat_280kA() const { return hCovMat_280kA; }

//-----------------------------------------------------

void NDFD_Matrix::NormaliseETrue(Eigen::MatrixXd *MatrixND,
                                 Eigen::MatrixXd *MatrixFD,
  ...
  std::vector<std::pair<Eigen::MatrixXd *, Eigen::ArrayXd>> matrix_pair = {
      NDpair, FDpair};
  ...
  for (auto &mat : matrix_pair) {
    for (int col_it = 1; col_it <= (mat.first->cols() - 2); col_it++) {
      ...
      double eff = 0.01;
      if (std::isnormal(mat.second(col_it - 1)))
        eff = mat.second(col_it - 1);
      projY *= eff;
...
void NDFD_Matrix::ExtrapolateNDtoFD(
    PRISMReweightableSpectrum NDDataSpec, double POT, const int kA,
    Eigen::ArrayXd &&weights, osc::IOscCalc *calc, ana::SystShifts shift,
    Flavors::Flavors_t NDflav, Flavors::Flavors_t FDflav,
    Current::Current_t curr, Sign::Sign_t NDsign, Sign::Sign_t FDsign,
    Eigen::ArrayXXd NDefficiency, Eigen::ArrayXd FDefficiency) const {
  ...
  // Do not want to oscillate the MC in the FD matrix (ND is always
  // un-oscillated). The linear combination handles the oscillation, we just
  // want to correct for the different detector resolutions. May need to revisit
  // osc vs. no-osc FD smearing matrices.
  auto sMatrixND =
      NDPredInterps.at(GetNDConfigFromPred(NDflav, NDsign))
          ->PredictComponentSyst(calc, shift, NDflav, curr, NDsign);
  hMatrixND =
      ConvertArrayToMatrix(sMatrixND.GetEigen(POT), sMatrixND.GetBinnings());

  auto sMatrixFD =
      FDPredInterps.at(GetFDConfigFromPred(FDflav, FDsign))
          ->PredictComponentSyst(calc, shift, FDflav, curr, FDsign);
  hMatrixFD =
      ConvertArrayToMatrix(sMatrixFD.GetEigen(POT), sMatrixFD.GetBinnings());

  Eigen::MatrixXd PRISMND = NDDataSpec.GetEigen(POT);
  Eigen::MatrixXd PRISMND_block =
      PRISMND.block(1, 1, PRISMND.rows() - 2, PRISMND.cols() - 2);
  ...
  Eigen::MatrixXd TotalLCCovMat =
      Eigen::MatrixXd::Zero(hMatrixFD.rows(), hMatrixFD.rows());
  ...
  for (int slice = 0; slice < PRISMND_block.rows(); slice++) {
    ...
    NormaliseETrue(&hMatrixND, &hMatrixFD, NDefficiency.row(slice),
                   FDefficiency);
    // Do Linear algebra without under/over-flow bins after normalisation.
    Eigen::MatrixXd MatrixND_block =
        hMatrixND.block(1, 1, hMatrixND.rows() - 2, hMatrixND.cols() - 2);
    Eigen::MatrixXd MatrixFD_block =
        hMatrixFD.block(1, 1, hMatrixFD.rows() - 2, hMatrixFD.cols() - 2);
    ...
    Eigen::MatrixXd CovMatRec =
        Eigen::MatrixXd::Zero(NDERec.size(), NDERec.size());
    ...
    Eigen::MatrixXd D =
        (MatrixND_block.transpose() * invCovMatRec * MatrixND_block).inverse() *
    ...
    Eigen::MatrixXd CovMatExtrap =
        MatrixFD_block * CovMatTrue * MatrixFD_block.transpose();
...
size_t NDFD_Matrix::GetNDConfigFromPred(Flavors::Flavors_t NDflav,
                                        Sign::Sign_t NDsign,
                                        bool is280kA) const {
  size_t conf;
  assert(NDflav == Flavors::kAllNuMu); // Only considering numu at ND.
  if (!is280kA)
    conf = (NDsign == Sign::kNu) ? kND_293kA_nu_numu : kND_293kA_nub_numub;
  else
    conf = (NDsign == Sign::kNu) ? kND_280kA_nu_numu : kND_280kA_nub_numub;
  return conf;
}

//----------------------------------------------------

size_t NDFD_Matrix::GetFDConfigFromPred(Flavors::Flavors_t FDflav,
                                        Sign::Sign_t FDsign) const {
  size_t conf;
  if (FDflav == Flavors::kNuMuToNuMu) {
    conf = (FDsign == Sign::kNu) ? kFD_nu_numu : kFD_nub_numub;
  } else if (FDflav == Flavors::kNuMuToNuE) {
    conf = (FDsign == Sign::kNu) ? kFD_nu_nue : kFD_nub_nueb;
  } else {
    abort();
  }
...
void NDFD_Matrix::Write(TDirectory *dir) const {
  ...
  osc::NoOscillations no;
  size_t confnd(kND_293kA_nu_numu);
  if (!NDPredInterps.at(confnd)) {
    confnd = kND_293kA_nub_numub;
  }
  auto sMND = NDPredInterps.at(confnd)->Predict(&no);
  size_t conf(0);
  if (!FDPredInterps.at(conf))
    conf = 1;
  if (!FDPredInterps.at(conf))
    conf = 4;
