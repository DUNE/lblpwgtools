...
# ROOT tries to steal the command-line arguments from Python :(
oldargs = sys.argv
sys.argv = []
...
# prelim.Draw()
def preliminary():
    prelim = TLatex(.9, .95, "NOvA Preliminary")
...
# prelim.Draw()
def preliminaryside():
    prelim = TLatex(.93, .9, "NOvA Preliminary")
    prelim.SetTextColor(4)
    prelim.SetNDC()
    prelim.SetTextSize(2 / 30.)
...
# sim.Draw()
def simulation():
    prelim = TLatex(.9, .95, "NOvA Simulation")
...
# sim.Draw()
def simulationside():
    prelim = TLatex(.93, .9, "NOvA Simulation")
    prelim.SetTextColor(921)
    prelim.SetNDC()
    prelim.SetTextSize(2 / 30.)
    prelim.SetTextAngle(270)
