...
void numuCC_selection_raw(float delta_r, bool momentum_method)
{
  ...
  TFile *fout = new TFile("numuCC_selection_raw.root","RECREATE");
  ...
  const SRCut kRecoCut  = kIsContainedNuMuCCRaw(delta_r, momentum_method);
  ...
  fout->cd();
  TString dir_name = TString::Format("delta_r_%.2f,momentum_method_%i", delta_r, momentum_method);
  fout->mkdir(dir_name);
  fout->cd(dir_name);
