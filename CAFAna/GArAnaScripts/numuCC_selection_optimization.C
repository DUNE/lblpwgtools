...
// cafe numuCC_selection_optimization.C
...
void numuCC_selection_optimization(float muon_score_cut, float delta_r, bool momentum_method, int sign_selection)
{
  ...
  TFile *fout = new TFile("numuCC_selection_optimization.root","RECREATE");
  ...
  const SRCut kTruthCutANuMu = kIsTrueANuMuCC && kIsTrueContained(delta_r);
  const SRCut kTruthCutNC = kIsTrueNC && kIsTrueContained(delta_r);
  const SRCut kTruthNotContained = !kIsTrueContained(delta_r);

  Spectrum sAll(loader[kIsNoCut].NuTruths(), axisTrueE);
  ...
  // Additional histogram to understand the sources of contamination
  Spectrum sANuMuCC(loader[kRecoCut && kTruthCutANuMu].NuTruths(), axisTrueE);
  Spectrum sNC(loader[kRecoCut && kTruthCutNC].NuTruths(), axisTrueE);
  Spectrum sNotContained(loader[kRecoCut && kTruthNotContained].NuTruths(), axisTrueE);

  loader.Go();

  // Grab POT from spectrum with no cuts
  const double pot = sAll.POT();

  fout->cd();
  TString dir_name = TString::Format("muon_score_%.2f,delta_r_%.2f,momentum_method_%i,sign_selection_%i,000", muon_score_cut, delta_r, momentum_method, sign_selection);
  fout->mkdir(dir_name);
  fout->cd(dir_name);
  ...
  // Get the other histograms as well
  TH1D* hANuMuCC(sANuMuCC.ToTH1(pot));
  hANuMuCC->SetName("ANuMuCC");
  hANuMuCC->Write("ANuMuCC", TObject::kWriteDelete);

  TH1D* hNC(sNC.ToTH1(pot));
  hNC->SetName("NC");
  hNC->Write("NC", TObject::kWriteDelete);

  TH1D* hNotContained(sNotContained.ToTH1(pot));
  hNotContained->SetName("NotContained");
  hNotContained->Write("NotContained", TObject::kWriteDelete);
