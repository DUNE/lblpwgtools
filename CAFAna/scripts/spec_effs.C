...
const Var kPIDFD_NUMU = SIMPLEVAR(cvnnumu);
const Var kPIDFD_NUE = SIMPLEVAR(cvnnue);
const Var kPID_MVA_NUMU = SIMPLEVAR(mvanumu);

const Var kvtxx_truth = SIMPLEVAR(vtx_x);
const Var kvtxy_truth = SIMPLEVAR(vtx_y);
const Var kvtxz_truth = SIMPLEVAR(vtx_z);

//All the vars for efficiency studies
const Var kNuMomX = SIMPLEVAR(NuMomX);
const Var kNuMomY = SIMPLEVAR(NuMomY);
const Var kNuMomZ = SIMPLEVAR(NuMomZ);
const Var kEv = SIMPLEVAR(Ev);
const Var kMode = SIMPLEVAR(mode);
const Var kLepMomX = SIMPLEVAR(LepMomX);
const Var kLepMomY = SIMPLEVAR(LepMomY);
const Var kLepMomZ = SIMPLEVAR(LepMomZ);
const Var kLepE = SIMPLEVAR(LepE);
const Var kLepNuAngle = SIMPLEVAR(LepNuAngle);
const Var kQ2 = SIMPLEVAR(Q2);
const Var kW = SIMPLEVAR(W);
//const Var kX = SIMPLEVAR(X);
const Var kY = SIMPLEVAR(Y);
const Var knP = SIMPLEVAR(nP);
const Var knN = SIMPLEVAR(nN);
const Var knipip = SIMPLEVAR(nipip);
const Var knipim = SIMPLEVAR(nipim);
const Var knipi0 = SIMPLEVAR(nipi0);
//const Var knikp = SIMPLEVAR(nikp);
//const Var knikm = SIMPLEVAR(nikm);
//const Var knik0 = SIMPLEVAR(nik0);
//const Var kniem = SIMPLEVAR(niem);
//const Var kniother = SIMPLEVAR(niother);
//const Var knNucleus = SIMPLEVAR(nNucleus);
//const Var knUNKNOWN = SIMPLEVAR(nUNKNOWN);
const Var kRecoLepEnNue = SIMPLEVAR(RecoLepEnNue);
const Var kRecoHadEnNue = SIMPLEVAR(RecoHadEnNue);
...
void spec_effs(bool reload = false)
{
  ...
  if(reload || TFile(stateFname).IsZombie()){
    ...
    predFDNumuFHC_Fid.SaveTo(&fout, "fd_numu_fhc_fid");
    predFDNueFHC_Fid.SaveTo(&fout, "fd_nue_fhc_fid");
    predFDNumuRHC_Fid.SaveTo(&fout, "fd_numu_rhc_fid");
    predFDNueRHC_Fid.SaveTo(&fout, "fd_nue_rhc_fid");

    predFDNueFHC_ev.SaveTo(&fout, "fd_nue_fhc_ev");
    predFDNueFHC_ev_fid.SaveTo(&fout, "fd_nue_fhc_ev_fid");
    predFDNueRHC_ev.SaveTo(&fout, "fd_nue_rhc_ev");
    predFDNueRHC_ev_fid.SaveTo(&fout, "fd_nue_rhc_ev_fid");
    predFDNueFHC_numomx.SaveTo(&fout, "fd_nue_fhc_numomx");
    predFDNueFHC_numomx_fid.SaveTo(&fout, "fd_nue_fhc_numomx_fid");
    predFDNueRHC_numomx.SaveTo(&fout, "fd_nue_rhc_numomx");
    predFDNueRHC_numomx_fid.SaveTo(&fout, "fd_nue_rhc_numomx_fid");
    predFDNueFHC_numomy.SaveTo(&fout, "fd_nue_fhc_numomy");
    predFDNueFHC_numomy_fid.SaveTo(&fout, "fd_nue_fhc_numomy_fid");
    predFDNueRHC_numomy.SaveTo(&fout, "fd_nue_rhc_numomy");
    predFDNueRHC_numomy_fid.SaveTo(&fout, "fd_nue_rhc_numomy_fid");
    predFDNueFHC_numomz.SaveTo(&fout, "fd_nue_fhc_numomz");
    predFDNueFHC_numomz_fid.SaveTo(&fout, "fd_nue_fhc_numomz_fid");
    predFDNueRHC_numomz.SaveTo(&fout, "fd_nue_rhc_numomz");
    predFDNueRHC_numomz_fid.SaveTo(&fout, "fd_nue_rhc_numomz_fid");
    predFDNueFHC_lepmomx.SaveTo(&fout, "fd_nue_fhc_lepmomx");
    predFDNueFHC_lepmomx_fid.SaveTo(&fout, "fd_nue_fhc_lepmomx_fid");
    predFDNueRHC_lepmomx.SaveTo(&fout, "fd_nue_rhc_lepmomx");
    predFDNueRHC_lepmomx_fid.SaveTo(&fout, "fd_nue_rhc_lepmomx_fid");
    predFDNueFHC_lepmomy.SaveTo(&fout, "fd_nue_fhc_lepmomy");
    predFDNueFHC_lepmomy_fid.SaveTo(&fout, "fd_nue_fhc_lepmomy_fid");
    predFDNueRHC_lepmomy.SaveTo(&fout, "fd_nue_rhc_lepmomy");
    predFDNueRHC_lepmomy_fid.SaveTo(&fout, "fd_nue_rhc_lepmomy_fid");
    predFDNueFHC_lepmomz.SaveTo(&fout, "fd_nue_fhc_lepmomz");
    predFDNueFHC_lepmomz_fid.SaveTo(&fout, "fd_nue_fhc_lepmomz_fid");
    predFDNueRHC_lepmomz.SaveTo(&fout, "fd_nue_rhc_lepmomz");
    predFDNueRHC_lepmomz_fid.SaveTo(&fout, "fd_nue_rhc_lepmomz_fid");

    predFDNueFHC_lepe.SaveTo(&fout, "fd_nue_fhc_lepe");
    predFDNueFHC_lepe_fid.SaveTo(&fout, "fd_nue_fhc_lepe_fid");
    predFDNueRHC_lepe.SaveTo(&fout, "fd_nue_rhc_lepe");
    predFDNueRHC_lepe_fid.SaveTo(&fout, "fd_nue_rhc_lepe_fid");
    predFDNueFHC_lepnuangle.SaveTo(&fout, "fd_nue_fhc_lepnuangle");
    predFDNueFHC_lepnuangle_fid.SaveTo(&fout, "fd_nue_fhc_lepnuangle_fid");
    predFDNueRHC_lepnuangle.SaveTo(&fout, "fd_nue_rhc_lepnuangle");
    predFDNueRHC_lepnuangle_fid.SaveTo(&fout, "fd_nue_rhc_lepnuangle_fid");
    predFDNueFHC_mode.SaveTo(&fout, "fd_nue_fhc_mode");
    predFDNueFHC_mode_fid.SaveTo(&fout, "fd_nue_fhc_mode_fid");
    predFDNueRHC_mode.SaveTo(&fout, "fd_nue_rhc_mode");
    predFDNueRHC_mode_fid.SaveTo(&fout, "fd_nue_rhc_mode_fid");
    predFDNueFHC_truex.SaveTo(&fout, "fd_nue_fhc_truex");
    predFDNueFHC_truex_fid.SaveTo(&fout, "fd_nue_fhc_truex_fid");
    predFDNueRHC_truex.SaveTo(&fout, "fd_nue_rhc_truex");
    predFDNueRHC_truex_fid.SaveTo(&fout, "fd_nue_rhc_truex_fid");
    predFDNueFHC_truey.SaveTo(&fout, "fd_nue_fhc_truey");
    predFDNueFHC_truey_fid.SaveTo(&fout, "fd_nue_fhc_truey_fid");
    predFDNueRHC_truey.SaveTo(&fout, "fd_nue_rhc_truey");
    predFDNueRHC_truey_fid.SaveTo(&fout, "fd_nue_rhc_truey_fid");
    predFDNueFHC_truez.SaveTo(&fout, "fd_nue_fhc_truez");
    predFDNueFHC_truez_fid.SaveTo(&fout, "fd_nue_fhc_truez_fid");
    predFDNueRHC_truez.SaveTo(&fout, "fd_nue_rhc_truez");
    predFDNueRHC_truez_fid.SaveTo(&fout, "fd_nue_rhc_truez_fid");


    predFDNueFHC_q2.SaveTo(&fout, "fd_nue_fhc_q2");
    predFDNueFHC_q2_fid.SaveTo(&fout, "fd_nue_fhc_q2_fid");
    predFDNueRHC_q2.SaveTo(&fout, "fd_nue_rhc_q2");
    predFDNueRHC_q2_fid.SaveTo(&fout, "fd_nue_rhc_q2_fid");
    predFDNueFHC_w.SaveTo(&fout, "fd_nue_fhc_w");
    predFDNueFHC_w_fid.SaveTo(&fout, "fd_nue_fhc_w_fid");
    predFDNueRHC_w.SaveTo(&fout, "fd_nue_rhc_w");
    predFDNueRHC_w_fid.SaveTo(&fout, "fd_nue_rhc_w_fid");
    predFDNueFHC_y.SaveTo(&fout, "fd_nue_fhc_y");
    predFDNueFHC_y_fid.SaveTo(&fout, "fd_nue_fhc_y_fid");
    predFDNueRHC_y.SaveTo(&fout, "fd_nue_rhc_y");
    predFDNueRHC_y_fid.SaveTo(&fout, "fd_nue_rhc_y_fid");
    predFDNueFHC_np.SaveTo(&fout, "fd_nue_fhc_np");
    predFDNueFHC_np_fid.SaveTo(&fout, "fd_nue_fhc_np_fid");
    predFDNueRHC_np.SaveTo(&fout, "fd_nue_rhc_np");
    predFDNueRHC_np_fid.SaveTo(&fout, "fd_nue_rhc_np_fid");
    predFDNueFHC_nn.SaveTo(&fout, "fd_nue_fhc_nn");
    predFDNueFHC_nn_fid.SaveTo(&fout, "fd_nue_fhc_nn_fid");
    predFDNueRHC_nn.SaveTo(&fout, "fd_nue_rhc_nn");
    predFDNueRHC_nn_fid.SaveTo(&fout, "fd_nue_rhc_nn_fid");
    predFDNueFHC_nipip.SaveTo(&fout, "fd_nue_fhc_nipip");
    predFDNueFHC_nipip_fid.SaveTo(&fout, "fd_nue_fhc_nipip_fid");
    predFDNueRHC_nipip.SaveTo(&fout, "fd_nue_rhc_nipip");
    predFDNueRHC_nipip_fid.SaveTo(&fout, "fd_nue_rhc_nipip_fid");
    predFDNueFHC_nipim.SaveTo(&fout, "fd_nue_fhc_nipim");
    predFDNueFHC_nipim_fid.SaveTo(&fout, "fd_nue_fhc_nipim_fid");
    predFDNueRHC_nipim.SaveTo(&fout, "fd_nue_rhc_nipim");
    predFDNueRHC_nipim_fid.SaveTo(&fout, "fd_nue_rhc_nipim_fid");
    predFDNueFHC_nipi0.SaveTo(&fout, "fd_nue_fhc_nipi0");
    predFDNueFHC_nipi0_fid.SaveTo(&fout, "fd_nue_fhc_nipi0_fid");
    predFDNueRHC_nipi0.SaveTo(&fout, "fd_nue_rhc_nipi0");
    predFDNueRHC_nipi0_fid.SaveTo(&fout, "fd_nue_rhc_nipi0_fid");
    predFDNueFHC_recolepen.SaveTo(&fout, "fd_nue_fhc_recolepen");
    predFDNueFHC_recolepen_fid.SaveTo(&fout, "fd_nue_fhc_recolepen_fid");
    predFDNueRHC_recolepen.SaveTo(&fout, "fd_nue_rhc_recolepen");
    predFDNueRHC_recolepen_fid.SaveTo(&fout, "fd_nue_rhc_recolepen_fid");
    predFDNueFHC_recohaden.SaveTo(&fout, "fd_nue_fhc_recohaden");
    predFDNueFHC_recohaden_fid.SaveTo(&fout, "fd_nue_fhc_recohaden_fid");
    predFDNueRHC_recohaden.SaveTo(&fout, "fd_nue_rhc_recohaden");
    predFDNueRHC_recohaden_fid.SaveTo(&fout, "fd_nue_rhc_recohaden_fid");
  ...
  PredictionNoExtrap& predFDNumuFHC_Fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_numu_fhc_fid").release();
  PredictionNoExtrap& predFDNueFHC_Fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_fid").release();
  PredictionNoExtrap& predFDNumuRHC_Fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_numu_rhc_fid").release();
  PredictionNoExtrap& predFDNueRHC_Fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_fid").release();

  PredictionNoExtrap& predFDNueFHC_ev = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_ev").release();
  PredictionNoExtrap& predFDNueFHC_ev_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_ev_fid").release();
  PredictionNoExtrap& predFDNueRHC_ev = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_ev").release();
  PredictionNoExtrap& predFDNueRHC_ev_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_ev_fid").release();

  PredictionNoExtrap& predFDNueFHC_NuMomX = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_numomx").release();
  PredictionNoExtrap& predFDNueFHC_NuMomX_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_numomx_fid").release();
  PredictionNoExtrap& predFDNueRHC_NuMomX = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_numomx").release();
  PredictionNoExtrap& predFDNueRHC_NuMomX_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_numomx_fid").release();
  PredictionNoExtrap& predFDNueFHC_NuMomY = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_numomy").release();
  PredictionNoExtrap& predFDNueFHC_NuMomY_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_numomy_fid").release();
  PredictionNoExtrap& predFDNueRHC_NuMomY = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_numomy").release();
  PredictionNoExtrap& predFDNueRHC_NuMomY_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_numomy_fid").release();
  PredictionNoExtrap& predFDNueFHC_NuMomZ = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_numomz").release();
  PredictionNoExtrap& predFDNueFHC_NuMomZ_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_numomz_fid").release();
  PredictionNoExtrap& predFDNueRHC_NuMomZ = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_numomz").release();
  PredictionNoExtrap& predFDNueRHC_NuMomZ_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_numomz_fid").release();

  PredictionNoExtrap& predFDNueFHC_LepMomX = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_lepmomx").release();
  PredictionNoExtrap& predFDNueFHC_LepMomX_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_lepmomx_fid").release();
  PredictionNoExtrap& predFDNueRHC_LepMomX = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_lepmomx").release();
  PredictionNoExtrap& predFDNueRHC_LepMomX_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_lepmomx_fid").release();
  PredictionNoExtrap& predFDNueFHC_LepMomY = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_lepmomy").release();
  PredictionNoExtrap& predFDNueFHC_LepMomY_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_lepmomy_fid").release();
  PredictionNoExtrap& predFDNueRHC_LepMomY = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_lepmomy").release();
  PredictionNoExtrap& predFDNueRHC_LepMomY_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_lepmomy_fid").release();
  PredictionNoExtrap& predFDNueFHC_LepMomZ = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_lepmomz").release();
  PredictionNoExtrap& predFDNueFHC_LepMomZ_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_lepmomz_fid").release();
  PredictionNoExtrap& predFDNueRHC_LepMomZ = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_lepmomz").release();
  PredictionNoExtrap& predFDNueRHC_LepMomZ_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_lepmomz_fid").release();
  PredictionNoExtrap& predFDNueFHC_LepE = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_lepe").release();
  PredictionNoExtrap& predFDNueFHC_LepE_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_lepe_fid").release();
  PredictionNoExtrap& predFDNueRHC_LepE = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_lepe").release();
  PredictionNoExtrap& predFDNueRHC_LepE_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_lepe_fid").release();
  PredictionNoExtrap& predFDNueFHC_LepNuAngle = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_lepnuangle").release();
  PredictionNoExtrap& predFDNueFHC_LepNuAngle_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_lepnuangle_fid").release();
  PredictionNoExtrap& predFDNueRHC_LepNuAngle = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_lepnuangle").release();
  PredictionNoExtrap& predFDNueRHC_LepNuAngle_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_lepnuangle_fid").release();

  PredictionNoExtrap& predFDNueFHC_mode = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_mode").release();
  PredictionNoExtrap& predFDNueFHC_mode_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_mode_fid").release();
  PredictionNoExtrap& predFDNueRHC_mode = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_mode").release();
  PredictionNoExtrap& predFDNueRHC_mode_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_mode_fid").release();

  PredictionNoExtrap& predFDNueFHC_truex = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_truex").release();
  PredictionNoExtrap& predFDNueFHC_truex_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_truex_fid").release();
  PredictionNoExtrap& predFDNueRHC_truex = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_truex").release();
  PredictionNoExtrap& predFDNueRHC_truex_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_truex_fid").release();
  PredictionNoExtrap& predFDNueFHC_truey = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_truey").release();
  PredictionNoExtrap& predFDNueFHC_truey_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_truey_fid").release();
  PredictionNoExtrap& predFDNueRHC_truey = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_truey").release();
  PredictionNoExtrap& predFDNueRHC_truey_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_truey_fid").release();
  PredictionNoExtrap& predFDNueFHC_truez = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_truez").release();
  PredictionNoExtrap& predFDNueFHC_truez_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_truez_fid").release();
  PredictionNoExtrap& predFDNueRHC_truez = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_truez").release();
  PredictionNoExtrap& predFDNueRHC_truez_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_truez_fid").release();


  PredictionNoExtrap& predFDNueFHC_q2 = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_q2").release();
  PredictionNoExtrap& predFDNueFHC_q2_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_q2_fid").release();
  PredictionNoExtrap& predFDNueRHC_q2 = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_q2").release();
  PredictionNoExtrap& predFDNueRHC_q2_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_q2_fid").release();
  PredictionNoExtrap& predFDNueFHC_w = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_w").release();
  PredictionNoExtrap& predFDNueFHC_w_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_w_fid").release();
  PredictionNoExtrap& predFDNueRHC_w = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_w").release();
  PredictionNoExtrap& predFDNueRHC_w_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_w_fid").release();
  PredictionNoExtrap& predFDNueFHC_y = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_y").release();
  PredictionNoExtrap& predFDNueFHC_y_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_y_fid").release();
  PredictionNoExtrap& predFDNueRHC_y = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_y").release();
  PredictionNoExtrap& predFDNueRHC_y_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_y_fid").release();

  PredictionNoExtrap& predFDNueFHC_nP = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_np").release();
  PredictionNoExtrap& predFDNueFHC_nP_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_np_fid").release();
  PredictionNoExtrap& predFDNueRHC_nP = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_np").release();
  PredictionNoExtrap& predFDNueRHC_nP_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_np_fid").release();
  PredictionNoExtrap& predFDNueFHC_nN = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_nn").release();
  PredictionNoExtrap& predFDNueFHC_nN_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_nn_fid").release();
  PredictionNoExtrap& predFDNueRHC_nN = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_nn").release();
  PredictionNoExtrap& predFDNueRHC_nN_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_nn_fid").release();

  PredictionNoExtrap& predFDNueFHC_nipip = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_nipip").release();
  PredictionNoExtrap& predFDNueFHC_nipip_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_nipip_fid").release();
  PredictionNoExtrap& predFDNueRHC_nipip = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_nipip").release();
  PredictionNoExtrap& predFDNueRHC_nipip_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_nipip_fid").release();
  PredictionNoExtrap& predFDNueFHC_nipim = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_nipim").release();
  PredictionNoExtrap& predFDNueFHC_nipim_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_nipim_fid").release();
  PredictionNoExtrap& predFDNueRHC_nipim = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_nipim").release();
  PredictionNoExtrap& predFDNueRHC_nipim_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_nipim_fid").release();
  PredictionNoExtrap& predFDNueFHC_nipi0 = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_nipi0").release();
  PredictionNoExtrap& predFDNueFHC_nipi0_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_nipi0_fid").release();
  PredictionNoExtrap& predFDNueRHC_nipi0 = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_nipi0").release();
  PredictionNoExtrap& predFDNueRHC_nipi0_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_nipi0_fid").release();

  PredictionNoExtrap& predFDNueFHC_RecoLepEn = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_recolepen").release();
  PredictionNoExtrap& predFDNueFHC_RecoLepEn_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_recolepen_fid").release();
  PredictionNoExtrap& predFDNueRHC_RecoLepEn = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_recolepen").release();
  PredictionNoExtrap& predFDNueRHC_RecoLepEn_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_recolepen_fid").release();

  PredictionNoExtrap& predFDNueFHC_RecoHadEn = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_recohaden").release();
  PredictionNoExtrap& predFDNueFHC_RecoHadEn_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_fhc_recohaden_fid").release();
  PredictionNoExtrap& predFDNueRHC_RecoHadEn = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_recohaden").release();
  PredictionNoExtrap& predFDNueRHC_RecoHadEn_fid = *ana::LoadFrom<PredictionNoExtrap>(&fin, "fd_nue_rhc_recohaden_fid").release();
