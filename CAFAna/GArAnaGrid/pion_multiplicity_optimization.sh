...
OUTDIR=/pnfs/dune/scratch/users/${GRID_USER}/CAFAna/pion_multiplicity_optimization

# Let's rename the output file so it's unique in case we send multiple jobs
OUTFILE=pion_multiplicity_optimization_${CLUSTER}_${PROCESS}_$(date -u +%Y%m%d).root
...
input_file="${INPUT_TAR_DIR_LOCAL}/input_params_pion.dat"
line=`expr "${PROCESS}" + 1`
input_line=$(sed "${line}!d" ${input_file})
params_array=($input_line)

cafe -bq GArAnaScripts/pion_multiplicity_optimization.C ${params_array[0]} ${params_array[1]} ${params_array[2]} ${params_array[3]}

# Copy back outputs
if [ -f pion_multiplicity_optimization.root ]; then

    # Rename output file(s)...
    mv pion_multiplicity_optimization.root $OUTFILE
