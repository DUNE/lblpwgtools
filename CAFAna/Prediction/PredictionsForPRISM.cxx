...
namespace ana {
  ...
  // PredictionNDNoOsc Class:
 
  //----------------------------------------------------------------------
  PredictionNDNoOsc::PredictionNDNoOsc(SpectrumLoaderBase& loader,
                                       ...
                                       const SystShifts& shift,
                                       const Weight& wei)
    : PredictionNDNoOsc(loader, HistAxis(label, bins, var), cut, shift, wei)
  {}
  ...
  {
    fSpectrum_ptr = &fSpectrum;//std::unique_ptr<Spectrum>(&fSpectrum);
    fSpectrumNC_ptr = &fSpectrumNC;//std::unique_ptr<Spectrum>(&fSpectrumNC);
    fSpectrumNumu_ptr = &fSpectrumNumu;//std::unique_ptr<Spectrum>(&fSpectrumNumu);
    fSpectrumNumubar_ptr = &fSpectrumNumubar;//std::unique_ptr<Spectrum>(&fSpectrumNumubar);
    fSpectrumNue_ptr = &fSpectrumNue;//std::unique_ptr<Spectrum>(&fSpectrumNue);
    fSpectrumNuebar_ptr = &fSpectrumNuebar;//std::unique_ptr<Spectrum>(&fSpectrumNuebar);
  }

  //----------------------------------------------------------------------
  Spectrum PredictionNDNoOsc::PredictComponent(osc::IOscCalc* /*calc*/,
                                                         Flavors::Flavors_t flav,
                                                         Current::Current_t curr,
                                                         Sign::Sign_t sign) const {
    ...
    //PRISMSpectrum ret = fSpectrum;
    Spectrum ret = *fSpectrum_ptr;
    ret.Clear();
    ...
    if(flav & kNuMuToNuMu && sign & kNu)     ret += *fSpectrumNumu_ptr;
    if(flav & kNuMuToNuMu && sign & kAntiNu) ret += *fSpectrumNumubar_ptr;
    if(flav & kNuEToNuE   && sign & kNu)     ret += *fSpectrumNue_ptr;  
    if(flav & kNuEToNuE   && sign & kAntiNu) ret += *fSpectrumNuebar_ptr;

    return ret;
  }

  //----------------------------------------------------------------------
  void PredictionNDNoOsc::SaveTo(TDirectory* dir, const std::string& name) const {
    TDirectory* tmp = gDirectory;
  ...
  //----------------------------------------------------------------------
  std::unique_ptr<PredictionNDNoOsc> PredictionNDNoOsc::LoadFrom(TDirectory* dir, 
                                                                 const std::string& name) {
    dir = dir->GetDirectory(name.c_str()); // switch to subdir
    assert(dir);

    PredictionNDNoOsc* ret = new PredictionNDNoOsc(
      *ana::LoadFrom<PRISMSpectrum>(dir, "spect"),
      *ana::LoadFrom<PRISMSpectrum>(dir, "spect_nc"),
      *ana::LoadFrom<PRISMSpectrum>(dir, "spect_numu"),
      *ana::LoadFrom<PRISMSpectrum>(dir, "spect_numubar"),
      *ana::LoadFrom<PRISMSpectrum>(dir, "spect_nue"),
      *ana::LoadFrom<PRISMSpectrum>(dir, "spect_nuebar"));

    delete dir;

    return std::unique_ptr<PredictionNDNoOsc>(ret);
  }
