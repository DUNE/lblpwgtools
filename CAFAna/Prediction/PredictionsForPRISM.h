...
/// Prediction for ND predictions for PRISM.
/// Essentially just PredictionNoOsc, but using the PRISMSpectrum wrapper
/// class that allows the user to do OverridePOT(pot). This is needed for the
/// PRISM analysis and I want to limit divergence from the master branch.
/// TODO: split these predictions into their own file soon!
class PredictionNDNoOsc : public IPrediction {
public: 
  PredictionNDNoOsc(SpectrumLoaderBase &loader, const HistAxis &axis,
                    const Cut &cut, const SystShifts &shift = kNoShift,
                    const Weight &wei = kUnweighted);

  PredictionNDNoOsc(SpectrumLoaderBase &loader, const std::string &label,
                    const Binning &bins, const Var &var, const Cut &cut,
                    const SystShifts &shift = kNoShift,
                    const Weight &wei = kUnweighted);

  ~PredictionNDNoOsc() {
    fSpectrum_ptr = nullptr;
    fSpectrumNC_ptr = nullptr;
    fSpectrumNumu_ptr = nullptr;
    fSpectrumNumubar_ptr = nullptr;   
    fSpectrumNue_ptr = nullptr;
    fSpectrumNuebar_ptr = nullptr;
  }

  static std::unique_ptr<PredictionNDNoOsc> LoadFrom(TDirectory *dir, const std::string& name);
  virtual void SaveTo(TDirectory *dir, const std::string& name) const override;
  ...
  /*virtual Spectrum Predict(osc::IOscCalc *calc) const override { 
    std::cout << "Can't call this because I can't code." << std::endl;
    abort();
  }*/
  virtual Spectrum Predict(osc::IOscCalc * /*calc*/) const override {
    //return fSpectrum;
    return *fSpectrum_ptr;
  }
...
protected:
  PredictionNDNoOsc(const PRISMSpectrum &s, const PRISMSpectrum &sNC, const PRISMSpectrum &sNumu,
                    const PRISMSpectrum &sNumubar, const PRISMSpectrum &sNue,
                    const PRISMSpectrum &sNuebar)
      : fSpectrum(s), fSpectrumNC(sNC), fSpectrumNumu(sNumu),
        fSpectrumNumubar(sNumubar), fSpectrumNue(sNue),
        fSpectrumNuebar(sNuebar) {}

  PRISMSpectrum fSpectrum;
  /*std::unique_ptr<Spectrum>*/Spectrum *fSpectrum_ptr;
  
  PRISMSpectrum fSpectrumNC;
  /*std::unique_ptr<Spectrum>*/Spectrum *fSpectrumNC_ptr;
  PRISMSpectrum fSpectrumNumu;
  /*std::unique_ptr<Spectrum>*/Spectrum *fSpectrumNumu_ptr;
  PRISMSpectrum fSpectrumNumubar;
  /*std::unique_ptr<Spectrum>*/Spectrum *fSpectrumNumubar_ptr;
  PRISMSpectrum fSpectrumNue;
  /*std::unique_ptr<Spectrum>*/ Spectrum *fSpectrumNue_ptr;
  PRISMSpectrum fSpectrumNuebar;
  /*std::unique_ptr<Spectrum>*/ Spectrum *fSpectrumNuebar_ptr;
};

class NDNoOscPredictionGenerator : public IPredictionGenerator {
public:
  NDNoOscPredictionGenerator(HistAxis axis, Cut cut, Weight wei = kUnweighted)
      : fAxis(axis), fCut(cut), fWei(wei) {
  }
  ...
  Generate(Loaders &loaders,
    ...
    return std::unique_ptr<IPrediction>(
        new PredictionNDNoOsc(loader, fAxis, fCut, shiftMC, fWei));
  }
